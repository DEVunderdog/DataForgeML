...
3. **`NonlinearityTag.Unpredictable` (pre-routing guard)** → Median, unconditionally. Overrides MAR mechanism and all severity levels below. Recorded in signals. See ADR 0016.
4. `MARSuspect` → MICE (if Severe or multi-column MAR) → Regression (if High + `correlated_with` non-empty + size guards) → KNN (if High + no correlations, or Minor/Moderate + `KurtosisTag.Leptokurtic`/`SkewSeverity.Severe`, size-guarded) → Median
5. `NumericKind.BoundedDiscrete`: `NumericFlag.Bimodal` set → Cluster-Conditional Imputation (mode within assigned cluster; domain-constrained GMM Sampling if no features available); otherwise → Mode (unconditional — finite domain makes model-based imputation semantically invalid)
